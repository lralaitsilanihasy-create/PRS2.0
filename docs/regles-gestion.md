...
- Historique des échanges d'un dossier clôturé [Lecture]
  - ⚠️ **Règle ajoutée** : `GET /api/dossiers/{id}/historique-echanges` (accessible **PRMP** et **Contrôleur vérificateur**, + Admin) retourne, pour un dossier **`CLOTURE`** uniquement (sinon 403), la chronologie complète triée date croissante : observations du vérificateur (`t_verification` : date, vérificateur, texte, `obsLevees` — dont le passage final `obsLevees=true`) et rectifications de la PRMP (`t_audit_log` : date, PRMP, motif).

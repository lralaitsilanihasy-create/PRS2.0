...
| **Règle d'entrée métier** (`BadRequestException`) | ex. `POST /api/mon-compte/changer-mot-de-passe` avec ancien mot de passe incorrect ou nouveau identique à l'ancien ; `POST /api/marches` quand la **localité de la PRMP** est introuvable (mode indéterminable) | message explicite |
...
**Champs `RegisterPrmpRequest`** (corps de `/register/prmp`)
...
| POST | /api/auth/register/prmp | `RegisterPrmpRequest` | `RegisterResponse` | 201, 400, 409 | PUBLIC |

> `POST /api/auth/register/prmp` crée la fiche PRMP **et** un compte **inactif**. La connexion
> reste refusée (**401** « Compte désactivé ») tant que l'Administrateur n'a pas validé le compte
> (voir **Comptes d'authentification**). Login ou identifiant PRMP déjà utilisés → **409**.
> À chaque inscription, les **Administrateurs sont notifiés** (notification de type `NOUVELLE_INSCRIPTION`).
...
```json
{
  ...
  "telPrmp": "0330000050", "idLocalite": "ANT"
}
```
```json
{
...
## Entités-PRMP
**Ressource** `/api/prmp-entites` — Lecture / écriture : tout utilisateur authentifié.

**Champs `PrmpEntiteDto`**

| Champ (JSON) | Type | Obligatoire | Contraintes |
|---|---|---|---|
| idPrmpEntite | number | Oui (PK, au POST) | clé primaire |
| idPrmp | string | Oui | @NotBlank, max 10 |
| idEntiteContract | number | Oui | @NotNull |
| dateAffectation | string (date) | Non | |
| actif | boolean | Oui | @NotNull |
...
| Méthode | URL | Corps | Réponse | Statuts | Rôle |
|---|---|---|---|---|---|
| GET | /api/prmp-entites | — | `PrmpEntiteDto[]` | 200 | Authentifié |
| GET | /api/prmp-entites/{id} | — | `PrmpEntiteDto` | 200, 404 | Authentifié |
| POST | /api/prmp-entites | `PrmpEntiteDto` | `PrmpEntiteDto` | 201, 400 | Authentifié |
| PUT | /api/prmp-entites/{id} | `PrmpEntiteDto` | `PrmpEntiteDto` | 200, 400, 404 | Authentifié |
| DELETE | /api/prmp-entites/{id} | — | — | 204, 404 | Authentifié |

`{id}` = idPrmpEntite (number).

**Exemple — requête**
```json
{ "idPrmpEntite": 45, "idPrmp": "PRMP001", "idEntiteContract": 8, "dateAffectation": "2026-01-20", "actif": true }
```
...
> et la **localité de la PRMP** (`marché → PPM → PRMP.idLocalite`) — via `t_regle_passation`/`t_seuil` ;
> toute valeur `idMode` envoyée par le client est **ignorée**. À la **mise à jour**, le mode est
> **recalculé** uniquement si `idNature`, `montEstim`, `idSituation` ou `idPpm` change.
> Si **aucune règle** ne correspond → le marché est créé avec `idMode = null` et une notification
> `MODE_NON_DETERMINE` est émise à la PRMP. Si la **localité de la PRMP est introuvable** → **400**.
